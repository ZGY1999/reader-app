...
当前实现已经完成 `v0.1 阅读底座公开试用版`、`v0.2 标注公开试用版` 和 `v0.3 AI 问书公开试用版`，下一阶段进入 `v0.4 TTS 公开试用版`。
...
1. 进入 `v0.4 TTS`
2. 在稳定阅读、标注、AI 底座上接入朗读、播放控制和高亮跟随
3. 继续收敛旧 TTS / AI UI 和服务代码

## 已知技术限制

- 开发模式依赖 Vite 跑在 `5174` 端口
- PDF 解析测试有 warning，需要后续处理 `standardFontDataUrl`
- 仓库里仍有未纳入当前公开试用范围的 TTS 旧 UI / 服务代码，后续需要继续收敛
